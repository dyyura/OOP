...
class Airplane:
     ...
     def init(self,model,year,max_speed): #идем по методу инициализации
         self.model = model
         self.year = year
     ...
     def take_off(self): #этот  метод должен изменить ис флайинг на тру
        self.is_flying = True
        print(f'Самолет {self.model} взлетел, скорость {self.max_speed} км/ч, расстояние {self.odometer}')

     def fly(self,km): #вот этот метод должен принять расстояние полета и изменить наши показания километража, то есть одометра
        self.odometer += 100
        
     def land(self): #а этот метод должен изменить с лэнд на фолс
        self.is_flying = False

samolet = Airplane(
    model = 'Viktory RF-27', #здесь присваеваем значения
    year = '2018',
    max_speed=2500
)
